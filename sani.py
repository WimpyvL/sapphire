# sani.py - Sani Voice Assistant Core Application
import os
import sys
import time
import signal
...
import core.sani_logging
import logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)
from core.identity import PRODUCT_NAME, DEFAULT_PROMPT, env_get
...
class VoiceChatSystem:
    def __init__(self):
        ...
        self.provider_runtime = {
            "stt": {"status": "disabled", "reason": ""},
            "tts": {"status": "disabled", "reason": ""},
            "wakeword": {"status": "disabled", "reason": ""},
        }
    ...
    def _set_provider_runtime(self, provider: str, status: str, reason: str = ""):
        self.provider_runtime[provider] = {"status": status, "reason": reason}

    def _warn_capability(self, message: str):
    ...
    def _prime_default_prompt(self):
        try:
            ...
            if not prompt_name:
                ...
                if chat_defaults_path.exists():
                    with open(chat_defaults_path, 'r', encoding='utf-8') as f:
                        ...
                        prompt_name = defaults.get('prompt', DEFAULT_PROMPT)
                else:
            ...
            fallback_prompt = (
                f"You are {PRODUCT_NAME}! You are sharp, direct, and grounded. \n"
    ...
    def init_components(self):
        try:
            if config.WAKE_WORD_ENABLED:
                ...
                self._set_provider_runtime("wakeword", "ready")
                ...
                self._set_provider_runtime("wakeword", "disabled")
        except Exception as e:
            logger.error(f"Wake word initialization failed: {e}")
            self._warn_capability(f"Wake word unavailable - continuing without it: {e}")
            from core.wakeword.wakeword_null import NullAudioRecorder, NullWakeWordDetector
            self.wake_word_recorder = NullAudioRecorder()
            self.wake_detector = NullWakeWordDetector(None)
            self._set_provider_runtime("wakeword", "error", str(e))
    ...
    def switch_stt_provider(self, provider_name: str):
        ...
        if not provider_name or provider_name == 'none':
            ...
            self._set_provider_runtime("stt", "disabled")
            return True
        ...
        try:
            ...
            if provider_name == 'sapphire_router':
                ...
                try:
                    ...
                    self._warn_capability(f"Microphone unavailable - STT will work via web UI only: {mic_err}")
                    self.whisper_recorder = NullAudioRecorder()
            self._set_provider_runtime("stt", "ready")
            ...
            self._set_provider_runtime("stt", "error", str(e))
            return False
    ...
    def _init_tts_provider(self, provider_name, base_dir=None):
        ...
        if not provider_name or provider_name == 'none':
            ...
            self._set_provider_runtime("tts", "disabled")
        ...
        try:
            ...
            self._set_provider_runtime("tts", "ready")
            ...
            self._stop_kokoro_server()
            self.tts = TTSClient(provider=get_tts_provider('none'))
            self._set_provider_runtime("tts", "error", str(e))
            return False
    ...
    def start_background_services(self):
        ...
        if provider and provider != 'none':
            ...
            try:
                ...
                self._warn_capability(f"STT provider '{provider}' unavailable - starting Sani without speech recognition.")
                self.whisper_client = NullWhisperClient()
                self._set_provider_runtime("stt", "error", str(e))
            except RuntimeError as e:
                logger.error(f"Failed to initialize STT provider '{provider}': {e}")
                self._warn_capability(f"STT provider '{provider}' failed to initialize - Sani will continue in web-first mode.")
                self.whisper_client = NullWhisperClient()
                self._set_provider_runtime("stt", "error", str(e))
        else:
            self._set_provider_runtime("stt", "disabled")
...
def run():
    ...
    print(f"Starting {PRODUCT_NAME} Voice Chat System")
    try:
    ...
    print(f"Starting {PRODUCT_NAME} server")
    ...
    try:
        ...
        import os
        if env_get("SANI_DEV", "SAPPHIRE_DEV"):
            plugin_loader.start_watcher()
            logger.info("Plugin file watcher started (SANI_DEV/SAPPHIRE_DEV)")
        ...
        BOLD = '\033[1m'
        RESET = '\033[0m'
        # Keep startup banner ASCII-only so Windows cp1252 consoles don't crash before bind.
        print(f"\n{CYAN_BG}{BLACK}{BOLD} [ACTIVE] {PRODUCT_NAME.upper()} IS NOW ACTIVE: {url} {RESET}\n")

        logger.info(f"{PRODUCT_NAME} is running. Starting uvicorn server...")
